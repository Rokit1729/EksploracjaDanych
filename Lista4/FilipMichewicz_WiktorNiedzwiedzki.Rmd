...
mapping <- solve_LSAP(conf_mat, maximum = TRUE)
wyniki <- mapping[wyniki]

conf_mat <- table(wyniki, Glass$Type)
rownames(conf_mat) <- as.character(c(1:3, 5:7))
colnames(conf_mat) <- as.character(c(1:3, 5:7))

kable(conf_mat, caption = "Macierz błędów; metoda k-średnich")
```

Dokładność: `r round(sum(diag(conf_mat))/sum(conf_mat)*100, 2)`%.
